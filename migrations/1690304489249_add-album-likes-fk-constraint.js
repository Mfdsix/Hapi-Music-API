...
const uniqueUserName = 'unique_album_likes.user'
const uniqueAlbumName = 'unique_album_likes.album'

exports.up = pgm => {
  pgm.addConstraint(tableName, uniqueUserName, 'UNIQUE(user_id)')
  pgm.addConstraint(tableName, fkUserName, 'FOREIGN KEY(user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE')
  pgm.addConstraint(tableName, uniqueAlbumName, 'UNIQUE(album_id)')
...
exports.down = pgm => {
  ...
  pgm.dropConstraint(tableName, uniqueAlbumName)
}
